...
Extract the job details following the schema. Be accurate with the role_family categorization based on the actual role requirements.`,
    ...
    // CRITICAL: Analysis data is essential - failure must block response
    ...
    if (analysisError) {
      console.error("Critical: Analysis insert failed:", analysisError)
      // Analysis data is mandatory - cannot return success without it
      return NextResponse.json(
        { 
          success: false, 
          error: "Failed to save job analysis data. Please try again.",
    ...
    // CRITICAL: Scores are essential for readiness gates - failure must block response
    ...
    if (scoresError) {
      console.error("Critical: Scores insert failed:", scoresError)
      // Scores are required for workflow gates - cannot return success without them
      return NextResponse.json(
        { 
          success: false, 
          error: "Failed to save job scoring data. Please try again.",
    ...
    // If orchestration failed, return error instead of success
    if (!orchestration.success) {
      return NextResponse.json(
        { 
          success: false, 
          error: "Job orchestration failed",
          orchestration_error: orchestration.generation?.error || "Unknown orchestration error",
